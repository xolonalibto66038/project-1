...
from apps.curriculum.models import Grade, Level, Specialty, Subject
...
class StudentOnboardingForm(forms.Form):
    ...
    grade = forms.ModelChoiceField(
        queryset=Grade.objects.select_related("level").order_by(
            "level__order", "order"
        ),
        label=_("Grade"),
    ...
    def clean(self):
        ...
        if grade:
            ...
            if not has_specialties and specialty:
                raise forms.ValidationError(_("This grade has no specialties."))
        return cleaned
