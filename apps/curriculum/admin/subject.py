...
class SubjectAdmin(admin.ModelAdmin):
    ...
    list_display        = ('name', 'short_name', 'level', 'icon_display', 'slug', 'grade_count', 'edit_button', 'delete_button')
    ...
    def icon_display(self, obj):
        if obj.icon:
            return format_html('<i class="{}" style="font-size:1.2rem;"></i> {}', obj.icon, obj.icon)
        return '—'
    ...
    def duplicate_view(self, request, pk):
        ...
        if obj:
            ...
            obj.save()
            self.message_user(request, _('Subject duplicated successfully.'), messages.SUCCESS)
