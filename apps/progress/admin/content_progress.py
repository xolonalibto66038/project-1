...
class ContentProgressAdmin(admin.ModelAdmin):
    ...
    # ── List view ──
    list_display        = (
        'student', 'target_type', 'object_id',
        'status_badge', 'first_viewed_at', 'completed_at',
    )
    ...
    fieldsets = (
        ...
        (_('Target'), {
            'fields': ('content_type', 'object_id', 'content_object_display', 'target_type'),
        ...
        (_('Timestamps'), {
            'fields': ('first_viewed_at', 'completed_at', 'created_at', 'updated_at'),
            'classes': ('collapse',),
        }),
    )
    ...
    def mark_completed(self, request, queryset):
        ...
        if skipped:
            self.message_user(
                request,
                _(f'{skipped} record(s) skipped (already completed or race condition).'),
                messages.WARNING,
            )
    ...
    def mark_incomplete(self, request, queryset):
        ...
        if skipped:
            self.message_user(
                request,
                _(f'{skipped} record(s) skipped (already incomplete or race condition).'),
                messages.WARNING,
            )
