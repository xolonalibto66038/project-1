...
class Course(TimeStampModel):
    ...
    chapter = models.ForeignKey(
        ...
        help_text=_(
            "The chapter this course belongs to. Leave blank for standalone courses."
        ),
    )
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=(
                    ...
                    | models.Q(
                        chapter__isnull=True, term__isnull=False
                    )  # standalone: term required
                ),
    ...
    def clean(self):
        ...
        if not self.chapter and not self.term:
            raise ValidationError(_("Standalone courses must have a term assigned."))
        ...
        if self.chapter:
            if self.term and self.term != self.chapter.term:
                raise ValidationError(
                    {
                        "term": _("Course term must match the chapter term (%(term)s).")
