...
class CourseAdmin(admin.ModelAdmin):
    ...
    ordering = (
        "chapter__grade_subject__grade__level__order",
        "chapter__order",
        "order",
    )
    list_per_page = 20
    ...
    def video_count(self, obj):
        ...
        url = (
            reverse("admin:content_videoresource_changelist")
            + f"?course__id__exact={obj.pk}"
        )
    ...
    def activate(self, request, queryset):
        ...
        self.message_user(
            request, _(f"{updated} course(s) activated."), messages.SUCCESS
        )
    ...
    def deactivate(self, request, queryset):
        ...
        self.message_user(
            request, _(f"{updated} course(s) deactivated."), messages.SUCCESS
        )
    ...
    def duplicate_view(self, request, pk):
        ...
        if obj:
            ...
            obj.save()
            self.message_user(
                request, _("Course duplicated successfully."), messages.SUCCESS
            )
