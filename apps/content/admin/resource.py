...
class ResourceAdmin(admin.ModelAdmin):
    ...
    # ── List view ──
    list_display        = (
        'title', 'resource_type', 'parent_display', 'status', #'status_badge',
        'difficulty', 'term', 'is_free', 'has_solution', 'file_display', 'order',
    )
    list_display_links  = ('title',)
    list_filter         = (
        'resource_type', 'status', 'difficulty', 'term',
        'is_free', 'has_solution',
        'course__chapter__grade_subject__grade__level',
        'course__chapter__grade_subject__grade',
        'course__chapter__grade_subject__subject',
    )
    search_fields       = ('title', 'slug', 'course__title', 'subject__name')
    ...
    fieldsets = (
        (_('Parent'), {
            ...
            'description': _(
                ...
                'directly to a subject (test/exam/past paper). Never both.'
            ),
        }),
        (_('Identity'), {
        ...
        (_('Files'), {
            ...
            'description': _('Upload the main resource file and optionally a separate solution file.'),
        }),
        (_('Metadata'), {
            'fields': ('metadata',),
            'classes': ('collapse',),
    ...
    def parent_display(self, obj):
        ...
        if obj.subject:
            url = reverse('admin:curriculum_subject_change', args=[obj.subject.pk])
    ...
    def publish(self, request, queryset):
        ...
        self.message_user(request, _(f'{updated} resource(s) published.'), messages.SUCCESS)

    @admin.action(description=_('Archive selected resources'))
    def archive(self, request, queryset):
        updated = queryset.update(status=ResourceStatus.ARCHIVED)
        self.message_user(request, _(f'{updated} resource(s) archived.'), messages.SUCCESS)
    ...
    def mark_draft(self, request, queryset):
        ...
        self.message_user(request, _(f'{updated} resource(s) set to draft.'), messages.SUCCESS)

    @admin.action(description=_('Mark selected as Free'))
    def mark_free(self, request, queryset):
        updated = queryset.update(is_free=True)
        self.message_user(request, _(f'{updated} resource(s) marked as free.'), messages.SUCCESS)
    ...
    def duplicate_view(self, request, pk):
        ...
        if obj:
            ...
            obj.save()
            self.message_user(request, _('Resource duplicated as draft.'), messages.SUCCESS)
